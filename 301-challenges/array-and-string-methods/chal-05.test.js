...
const templateWithJQuery = () => {
  // Solution code here...
  $('#template').clone()
  .text(starWarsPeople.forEach((ele)=> $('h2').append( ele.name)))
...
const listFoods = (recipe) => {
  ...
  for(let i=0; i<recipe.ingredients.length;i++){
    let one = recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ')+1)
    let two = one.slice(one.indexOf(' ') +1)
    result.push(two)
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  for(let i = 0; i < recipe.ingredients.length; i++){
    result.push(recipe.ingredients[i]
        .split(' ')
        .slice(2, recipe.ingredients[i].length)
        .join(' ')
        )
  }
  return result;
...
const stepActions = (recipe) => {
  ...
  for(let i=0; i<recipe.steps.length;i++){
    ...
    result.push(recipe.steps[i].slice(recipe.steps[i][0], recipe.steps[i].indexOf(' ')))
  }
  return result;
};
...
const extractVowels = (str) => {
  ...
  let vowRemoved = str.match(regex).sort().join('')
  return [removed, vowRemoved]
};
